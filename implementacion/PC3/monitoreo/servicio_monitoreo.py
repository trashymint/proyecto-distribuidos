...
TIMEOUT_MS = 3000  # 3 segundos para detectar que PC3 está caído
...
def enviar_query_db(context, peticion):
    ...
    Si no responde en el timeout, usa la BD Réplica (PC2).
    """
    # Intentar BD Principal
    try:
        ...
        s.close()
        print("[MONITOREO] ⚠  PC3 no disponible — usando BD Réplica (PC2)...")

    # Fallback a BD Réplica
    try:
...
def imprimir_resultados(resp, fuente):
    ...
    if isinstance(datos, dict) and not datos.get("exito") is False:
        # Estado en memoria (devuelto por analítica) — dict de intersecciones
...
def main():
    ...
    while True:
        ...
        if cmd == "estado" and len(partes) >= 2:
            ...
            # Primero pedir a la analítica (estado en memoria, más fresco)
