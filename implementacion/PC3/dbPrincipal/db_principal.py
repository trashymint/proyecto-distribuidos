...
def inicializar_bd():
    ...
    cursor.close()
    conn.close()
    print("[BD PRINCIPAL] Tablas verificadas/creadas correctamente")
...
def manejar_query(cursor, peticion):
    ...
    if tipo == "historico":
        ...
        return {"exito": True, "datos": resultados, "fuente": "principal"}
...
def main():
    ...
    socket_pull.bind(f"tcp://*:{P['analitica_db_principal']}")

    socket_rep  = context.socket(zmq.REP)
    socket_rep.bind(f"tcp://*:{P['db_principal_query']}")
    ...
    print("=" * 55)
    print("  BASE DE DATOS PRINCIPAL - PC3")
    print("=" * 55)
    print(f"  PULL eventos:  puerto {P['analitica_db_principal']}")
    print(f"  REP consultas: puerto {P['db_principal_query']}")
    print(f"  MySQL: {DB_CONF['host']}:{DB_CONF['port']} / {DB_CONF['database']}")
    print("=" * 55)

    while True:
        try:
            eventos_poll = dict(poller.poll())

            if socket_pull in eventos_poll:
                ...
                cur.close()
                conn.close()
                print(f"[BD PRINCIPAL] Guardado: {evento.get('topic','?')} — "
                      f"{evento.get('interseccion', evento.get('sensor_id','?'))}")

            if socket_rep in eventos_poll:
                ...
                print(f"[BD PRINCIPAL] Consulta atendida: {peticion.get('tipo')}")

        except mysql.connector.Error as e:
            print(f"[BD PRINCIPAL] Error MySQL: {e}")
        except Exception as e:
            print(f"[BD PRINCIPAL] Error: {e}")
